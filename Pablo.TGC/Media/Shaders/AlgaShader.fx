...
//Vertex Shader
VS_OUTPUT_DIFFUSE_MAP vs_DiffuseMap2(VS_INPUT_DIFFUSE_MAP input)
{ 
	VS_OUTPUT_DIFFUSE_MAP output;
	...
	float valor = (y + 85) / 90;
       	...
       	input.Position.x += cos(time)*valor*sign(input.Position.x);
       	input.Position.z += sin(time)*valor*sign(input.Position.z);
...
technique RenderScene2
{
   pass Pass_0
   {
	  VertexShader = compile vs_3_0 vs_DiffuseMap2();
	  PixelShader = compile ps_3_0 ps_DiffuseMap();
   }
}
