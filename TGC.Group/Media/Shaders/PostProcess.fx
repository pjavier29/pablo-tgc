...
float intensidad_ancho;
float intensidad_alto;
float lightIntensityRelitive; //Intensidad de la luz, recibimos un numero entre 0 y 1

//Pixel Shader de la LLuvia
float4 ps_rain( PS_INPUT_DEFAULT Input ) : COLOR0
{     
	//Obtener color de textura
	float4 color = tex2D( RenderTarget, Input.Texcoord );
	
	float4 colorFinal = sin(Input.Texcoord.x * intensidad_ancho) > 0.99
			? (sin(Input.Texcoord.y * intensidad_alto) > 0.0)
				?float4(0.6, 0.85, 0.92, 0.0) * lightIntensityRelitive
				:color
			: color;
	return colorFinal;
}
